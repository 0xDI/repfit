...
export default async function DashboardPage() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-background">
      ...
      <main className="mx-auto w-full max-w-2xl space-y-4 p-4 pb-12 md:max-w-7xl md:space-y-6 md:p-8">
        ...
        {adminMessages && adminMessages.length > 0 && (
          <div className="space-y-2">
            {adminMessages.map((message) => (
              <Alert key={message.id} className="rounded-xl border-border/40 bg-card/50 backdrop-blur-sm md:rounded-2xl">
                <Info className="h-4 w-4" />
                <AlertTitle className="text-sm font-semibold">{message.title}</AlertTitle>
                <AlertDescription className="text-xs text-muted-foreground">{message.message}</AlertDescription>
              </Alert>
            ))}
          </div>
        )}
          ...
          {hasNoSubscription && (
            <div className="flex items-center gap-3 p-4 rounded-xl border border-primary/30 bg-primary/5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Contact us to start booking sessions</p>
              </div>
              <a
                href="tel:+306937043559"
                className="shrink-0 inline-flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-xs font-bold text-primary-foreground hover:bg-primary/90 shadow-lg shadow-primary/20 active:scale-95"
              >
