...
export async function createBooking(sessionId: string) {
  ...
  const { data: session } = await adminClient.from("gym_sessions").select("*").eq("id", sessionId).single()
  ...
  if (session.gym_id) {
    const { data: sub } = await adminClient
      ...
      .order("gym_id", { ascending: false }) // Prioritize the exact gym match if both exist
      .limit(1)
      .maybeSingle()
      
    activeSubscription = sub
  } else {
    // Fallback if session has no gym_id
    const { data: sub } = await adminClient
      .from("user_subscriptions")
      .select("*")
      .eq("user_id", user.id)
...
export async function rescheduleBooking(bookingId: string, newSessionId: string) {
  ...
  // Update slots for both sessions
  const adminClient = await createAdminClient()
  
  // Increment old session
  await adminClient
    .from("gym_sessions")
    .update({ available_slots: oldSession.available_slots + 1 })
    .eq("id", oldSession.id)

  // Decrement new session
  await adminClient
    .from("gym_sessions")
    .update({ available_slots: newSession.available_slots - 1 })
    .eq("id", newSession.id)
